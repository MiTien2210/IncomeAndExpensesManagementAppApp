...
export default function TransactionDetailScreen() {
  ...
  const { loadTransactions, transactions } = useContext(TransactionContext);
  ...
  const transactionInStore = transactions.find((x) => x.id === transaction.id) || transaction;
  ...
  const handleDelete = async () => {
    try {
      setIsLoading(true)
      await deleteTransaction(transactionInStore.id);
    ...
    <View style={styles.container}>
      <View style={styles.card}>
        <Text style={styles.title}>
          {transactionInStore.type === 'income' ? `${ICON.INCOME} Thu nhập` : `${ICON.EXPENSE} Chi tiêu`}
        </Text>
        ...
        <View style={styles.item}>
          ...
          <Text style={styles.value}>{transactionInStore.note || 'Không có'}</Text>
        </View>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity style={styles.deleteButton} onPress={confirmDelete} disabled={isLoading}>
            {isLoading ? (
        ...
        </View>
        {
          isAdd &&
          <View>
            <TouchableOpacity style={{ backgroundColor: colors.main, marginTop: 20, padding: 10, borderRadius: 10 }} onPress={onFinish} disabled={isLoading}>
